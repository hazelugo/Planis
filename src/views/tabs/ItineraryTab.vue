...
function compareByTime(a: TripEvent, b: TripEvent) {
...
const groupedEvents = computed(() => {
  ...
  return [...map.entries()]
    ...
    .map(([, group]) => ({
      ...group,
      events: [...group.events].sort(compareByTime),
    }))
})

const matchCount = computed(() => groupedEvents.value.reduce((n, g) => n + g.events.length, 0))
  ...
  <div class="space-y-5 anim-fade-up">
    ...
    <div v-else class="space-y-4">
      ...
      <div class="print:hidden bg-surface rounded-2xl border border-slate-100 dark:border-hairline shadow-sm px-4 py-3 space-y-3">
        ...
        <p v-if="trip.state.events.length" class="text-xs text-slate-400 pt-1 border-t border-slate-50 dark:border-hairline">
            ...
            <div class="space-y-1">
              <template v-for="event in group.events">
                ...
                <div v-else :key="event.id"
                  class="relative flex items-start gap-0 group py-2 transition-all rounded-xl">
